...
- `bin/` — cross-OS scripts placed on `$PATH`.
  ...
  KakaoTalk used to live here too. It now has its own repo,
  [chaotic-ground/kakaotalk-on-wine](https://github.com/chaotic-ground/kakaotalk-on-wine),
  which `3rd-step.sh` clones and runs: the setup grew long, and almost
  none of it is personal — it is what Wine, Bottles and GNOME do about a
  Korean Windows app, which is no use to anybody buried in a dotfiles
  repo.
