...
4. **Smoke test.** Run `echo '{"prompt":"안녕하세요"}' | bash ~/.claude/scripts/translate-ko.sh` and wait ~15 seconds, then `cat ~/.claude/translations.log` to confirm a translation appeared. If it didn't, surface the failure (probably `claude -p` auth issue or model name change).
